...
def main():
    ...
    image_dir = "/Users/shonie/Documents/MS Data Analytics Engineering/Github_Repositories/flickr8k_images_dataset/Images/"
    ...
    output_file = os.path.join(current_script_dir, "error_taxonomy_sheet.xlsx")
...
def generate_evaluation_data(
    ...
    for i, img_id in tqdm(
        ...
        caption_greedy = generate_caption_greedy(model, tokenizer, image_features[i], max_len)
        caption_greedy = caption_greedy.replace(START_TOKEN, "").replace(END_TOKEN, "").strip()

        caption_beam = generate_caption_beam(model, tokenizer, image_features[i], max_len, beam_width=5)
        caption_beam = caption_beam.replace(START_TOKEN, "").replace(END_TOKEN, "").strip()

        data.append(
            {
                "image_file": img_id,
                "ground_truth_caption": gt_caption,
                "generated_caption": caption_greedy,
                "search_type": "Greedy",
            }
        )
        data.append(
            {
                "image_file": img_id,
                "ground_truth_caption": gt_caption,
                "generated_caption": caption_beam,
                "search_type": "Beam",
            }
        )
    return data
...
def create_evaluation_spreadsheet(
    image_dir, captions_data, output_filename="error_taxonomy.xlsx"
):
    wb = Workbook()
    ws = wb.active
    ...
    column_widths = [40, 40, 40, 10, 40, 15, 15, 15, 15, 15, 15, 15, 15]
    for col, width in zip("ABCDEFGHIJKLM", column_widths):
        ws.column_dimensions[col].width = width

    
    dv = DataValidation(
        type="list",
        formula1='"Yes,No"',  # comma-separated and wrapped in double-quotes
        allow_blank=True,
        showDropDown=False,
        showErrorMessage=True,
        errorTitle="Invalid Entry",
        error="Please enter Yes or No",
        promptTitle="Response",
    ...
    while i < len(captions_data):
        item_greedy = captions_data[i]
        item_beam = captions_data[i + 1]

        image_path = os.path.join(image_dir, item_greedy["image_file"])
        ...
        try:
            ws[f"A{row}"].value = os.path.basename(item_greedy["image_file"])
            ws.merge_cells(start_row=row, start_column=1, end_row=row + 1, end_column=1)
            ws[f"A{row}"].alignment = Alignment(wrapText=True, horizontal="center", vertical="center")
        
            # Add and merge image cell over two rows
            ...
            # Merge GT caption over two rows
            ws[f"C{row}"].value = item_greedy["ground_truth_caption"]
            ws.merge_cells(start_row=row, start_column=3, end_row=row + 1, end_column=3)
            ws[f"C{row}"].alignment = Alignment(wrapText=True, vertical="center")

            # Write Greedy row
            ws[f"E{row}"].value = item_greedy["generated_caption"]
            ws[f"D{row}"].value = item_greedy["search_type"]

            # Write Beam row
            ws[f"E{row+1}"].value = item_beam["generated_caption"]
            ws[f"D{row+1}"].value = item_beam["search_type"]

            # Alignment for captions
            for r in [row, row + 1]:
                for col in "ED":
                    ws[f"{col}{r}"].alignment = Alignment(wrapText=True, horizontal="center", vertical="center")

            # Add validation for error fields (columns F to M)
            center_wrap_alignment = Alignment(wrap_text=True, horizontal="center", vertical="center")
            for r in [row, row + 1]:
                for col_idx in range(6, 14):  # F to M
            ...
            logger.error(f"Failed to process image {item_greedy['image_file']}: {e}", exc_info=True)
            i += 2
            continue
